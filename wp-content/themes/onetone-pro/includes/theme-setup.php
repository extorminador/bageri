<?php
 function onetone_setup(){
	...
	register_nav_menus( array('onepage' => __( 'One Page Template Header Menu', 'onetone' )));
	register_nav_menus( array('shop' => __( 'Shop Header Menu', 'onetone' )));
...
add_action( 'after_setup_theme', 'onetone_setup' );
 ...
 function onetone_custom_scripts(){
	...
	wp_enqueue_style( 'onetone-main', get_stylesheet_uri(), array(), $onetone_theme->get( 'Version' ) );
   ...
   $onetone_custom_css = "";
	...
	$onetone_custom_css  .= ".home-header,.shop-header, .template-header{background-image:url(".$header_image. ");background-attachment: fixed;background-repeat:no-repeat;background-size:cover;-moz-background-size:cover;-webkit-background-size:cover;opacity:".$header_opacity.";}\n";
	}
	$onetone_custom_css  .= ".home-header, .template-header{opacity:".$header_opacity.";}\n";
	
    if ( 'blank' != get_header_textcolor() && '' != get_header_textcolor() ){
     $header_color        =  ' color:#' . get_header_textcolor() . ';';
	 $onetone_custom_css .=  '.home-header, .shop-header,.template-header,.site-name,.site-description{'.$header_color.'}';
		}
	...
	$onetone_custom_css  .=  'a,.site-logo a:hover,.site-navigation a:hover,.widget a:hover,.entry-title a:hover,.entry-meta a:hover,.loop-pagination a:hover,.page_navi a:hover,.site-footer a:hover,.home-navigation > ul > li.current > a > span,.home-navigation li a:hover,.home-navigation li.current a,.home-footer a:hover,#back-to-top,#back-to-top span,.time-title:hover{color:'.$links_color.' ;}mark,ins,#wp-calendar #today{background:'.$links_color.' ;}#back-to-top{border:1px solid '.$links_color.';}::selection{background:'.$links_color.' ;}::-moz-selection{background:'.$links_color.' ;}';
	...
	$video_full_screen         = onetone_options_array("video_full_screen");
	$video_full_screen         = is_numeric($video_full_screen)?$video_full_screen:"1";
	if( $video_full_screen != "1")
	$onetone_custom_css       .= '#tubular-container{ position:absolute!important;}';
	
	$footer_typography         = onetone_options_array( 'footer_typography');
	$_footer_typography        = onetone_get_typography( $footer_typography );
    $onetone_custom_css       .= 'footer .footer-copyright,.home-widget-area{'.$_footer_typography.'}';
   ...
   $onetone_custom_css        .=  'footer.home-footer{'.$_footer_background.'}';
	...
	wp_enqueue_script( 'onetone-default', get_template_directory_uri().'/js/onetone.js', array( 'jquery' ), $onetone_theme->get( 'Version' ), true );
	
	$slide_time         = onetone_options_array("slide_time");
	$slide_time         = is_numeric($slide_time)?$slide_time:"5000";
	
	
	$header_cover_video_background          = onetone_options_array( "header_cover_video_background" ,1);
	...
	wp_localize_script( 'onetone-default', 'onetone_params', array(
			...
			'header_cover_video_background'=> $header_cover_video_background
		)  );
	...
	if( $pagenow == "post.php" || $pagenow == "post-new.php" || (isset($_GET['page']) && $_GET['page'] == "onetone-options")){
	 ...
	 wp_enqueue_style('onetone-admin',  get_template_directory_uri() .'/css/admin.css', false, $onetone_theme->get( 'Version' ), false);
	 wp_enqueue_script( 'onetone-magnific', get_template_directory_uri().'/js/jquery.magnific-popup.min.js', array( 'jquery' ), '', true );
	 wp_enqueue_script( 'onetone-admin', get_template_directory_uri().'/js/admin.js', array( 'jquery' ), $onetone_theme->get( 'Version' ), true );
